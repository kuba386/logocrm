...
/**
 * Дата, которая заведомо не позже занятий фикстуры, — для полей «дата
 * начала» абонемента и «с» у заморозки.
 *
 * Фикстура и тест считают «вчера по Бишкеку» порознь: первая в psql при
 * посеве, второй в браузере минутой позже. Если между этими моментами
 * проходит полночь Бишкека (18:00 UTC), тест получает дату на день позже
 * фикстуры — и абонемент, датированный «вчера», оказывается позже занятий.
 * Дальше `s.starts_at <= v_lesson_date` (0015) просто перестаёт находить
 * кандидата на списание: ошибки нет, остаток не меняется. Ровно это
 * уронило прогон 19.09: фикстура легла в 23:59 по Бишкеку, тесты начались
 * в 00:00.
 *
 * День запаса закрывает гонку целиком и ровно: фикстура всегда применяется
 * ДО тестов, поэтому её дата не может оказаться позже тестовой, а разойтись
 * они могут максимум на одну полночь.
 *
 * Для навигации по неделе этот приём не годится — там нужна настоящая дата
 * занятий, иначе на стыке недель уедем на неделю назад. Навигация
 * по-прежнему на bishkekYesterdayIso().
 */
export function bishkekBeforeLessonsIso(): string {
  const d = new Date(Date.now() + 6 * 60 * 60 * 1000)
