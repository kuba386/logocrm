...
- «Доработка» п.2 и дата платежа — после ревью плана (`architect`, 15
  ...
  - **`0029_salary_runs_and_payment_date.sql` (а):**
    ...
    - **Дата платежа.** Старая 8-параметровая `record_payment` дропается,
      ...
      нижняя граница — не раньше `created_at` плательщика (опечатка годом
      не уезжает в 2024). `sell_subscription_paid` переводится на
      `p_paid_on` — одна конвертация вместо двух. Внутренние позиционные
      вызовы (8 аргументов, `pay_installment`) резолвятся в новую. После
      `drop` — гранты заново; 0007 — обе сигнатуры. Параметр въезжает
