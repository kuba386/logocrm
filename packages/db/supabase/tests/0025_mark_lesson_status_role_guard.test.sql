...
-- 2. Само занятие не тронуто. Без этой проверки регрессия «исключение есть,
-- но update всё равно прошёл» осталась бы незамеченной.
select is(
  (select status from public.lessons where id = '44444444-0025-0000-0000-000000000001'),
  'planned',
  'Занятие осталось planned — отозванный его не закрыл');

-- 3. 'planned' — тот же путь, тоже без emit_event.
select throws_ok(
...
-- 4. 'cancelled' раньше тоже отбивался, но через emit_event и с чужим текстом
