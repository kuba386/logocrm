...
select set_eq(
  ...
  $$ values ('attendance_statuses'), ('invitations') $$,
  'Таблицы с tenant_admin без решения по новым ролям — только attendance_statuses (read_all) и invitations; новая таблица роняет'
);
select is(
