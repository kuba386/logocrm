...
-- Занятие 31 августа 23:30 по Лиссабону — в UTC это уже 1 сентября.
-- Обязано попасть в АВГУСТ, а не в сентябрьский отчёт.
insert into public.lessons (id, center_id, teacher_id, student_id, service_id, status, starts_at, ends_at) values
  ...
  ('6fffffff-0000-0000-0000-000000000003','6ccccccc-0000-0000-0000-00000000000a','6aaaaaaa-0000-0000-0000-000000000001','6eeeeeee-0000-0000-0000-000000000001','6bbbbbbb-0000-0000-0000-000000000001','done','2026-08-31 23:30+01','2026-09-01 00:15+01');
...
select '6ccccccc-0000-0000-0000-00000000000a', l.id, '6eeeeeee-0000-0000-0000-000000000001',
  ...
  from public.lessons l where l.id in (
    ...
    '6fffffff-0000-0000-0000-000000000003');
...
select is(
  ...
  2, 'Сентябрь: два занятия — августовское в него не попало');

select is(
  (public.student_monthly_report('6eeeeeee-0000-0000-0000-000000000001','2026-08-01') ->> 'lessons_total')::int,
  1, 'Занятие 31 августа 23:30 по центру осталось в августе, хотя в UTC это сентябрь');
...
select is(
  ...
  70, 'Последняя оценка периода — а не текущая на момент вызова');
