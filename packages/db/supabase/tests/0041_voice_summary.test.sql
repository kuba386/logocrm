...
select is(
  (select count(*)::int from public.goal_progress where goal_id = '5bbb0000-0000-0000-0000-000000000002'),
  0, 'И ни одной строки прогресса по чужой цели не появилось');
...
select throws_ok(
  format($q$ select public.ai_write_lesson_note(%s,
    ...
    (select max(id) from public.events where type = 'lesson.voice_received')),
  '22023', null,
  'Оценка вне 0–100 отбивается: модель вернёт свою шкалу, если не проверить');
...
select is(
  ...
  'Дата прогресса — день занятия в поясе центра, не center_today воркера');
...
-- Идемпотентность и конфликт с ручной оценкой ------------------------------------------------------------
