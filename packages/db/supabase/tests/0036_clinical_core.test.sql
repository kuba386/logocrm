...
-- 17-25. Ведущий специалист видит своё и только своё ------------------------------------------------
...
-- 26-30. Другой специалист: отменённое занятие доступа не даёт (Р10) --------------------------------
...
-- 31-40. Родитель: только то, что для него -----------------------------------------------------------
...
-- 41-44. Черновик родителю не виден ни при каких условиях --------------------------------------------
...
-- 45-48. Чего родитель не увидит физически -------------------------------------------------------------
...
-- 49-56. Стойке и бухгалтеру клиника не положена (Р2) ---------------------------------------------------
...
-- 57-62. Владелец и администратор видят всё --------------------------------------------------------------
...
-- 63-67. Чужой центр (Database.md: обязательный тест для каждой таблицы с RLS) ------------------------------
...
select lives_ok(
  $q$ insert into public.homework_exercises (homework_id, exercise_id)
      values ('aaaa0000-0000-0000-0000-000000000001', 'dddd0000-0000-0000-0000-000000000001') $q$,
  'Упражнение платформы добавляется в ДЗ — составной FK это бы запретил (Р7)');

select lives_ok(
  $q$ insert into public.homework_exercises (homework_id, exercise_id)
      values ('aaaa0000-0000-0000-0000-000000000001', 'dddd0000-0000-0000-0000-000000000002') $q$,
...
select is((select count(*)::int from public.homework_exercises), 2,
  'Родитель видит состав задания своего ребёнка — иначе ДЗ приходит без названий упражнений');
reset role;
