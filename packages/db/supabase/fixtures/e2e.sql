...
-- фикстуры (2027), этой проверке никогда не пройдёт. Пять слотов по 45
-- минут, час между началами, все в прошлом на момент прогона: конец
-- последнего — за 15 минут до now(), начало первого — за 5 часов. Учитель
-- тот же, что у Айлин (Нургуль) — для пункта 5 чек-листа (специалист
-- отмечает своё).
--
-- greatest(..., v_midnight) — без него «начало первого — за 5 часов»
-- при прогоне в первые часы суток по Бишкеку уходит на вчерашнюю дату,
-- а sell_subscription по умолчанию берёт center_today() — «сегодня» по
-- тому же Бишкеку (0010:256). Тогда starts_at свежепроданного абонемента
-- (сегодня) позже даты первого занятия (вчера), кандидат для списания не
-- находится (s.starts_at <= v_lesson_date, 0010:861), и «Отметить
-- посещение» перестаёт списывать — тот же класс бага, что чинили в
-- fix/0010-freeze-test-timezone-flake, только тут расходятся между собой
-- две даты внутри одного прогона, а не внутри одного теста. greatest не
-- даёт занятию уйти раньше полуночи Бишкека, чем бы ни было now() —
-- впритык к самой полуночи несколько слотов могут схлопнуться в одну
-- отметку времени, но это не ломает ни «уже началось», ни совпадение
-- по дате с абонементом, только (крайне маловероятно) порядок между
-- ними самими.
do $$
declare
  v_midnight timestamptz :=
    date_trunc('day', now() at time zone 'Asia/Bishkek') at time zone 'Asia/Bishkek';
  v_l1 timestamptz := greatest(now() - interval '5 hours', v_midnight);
  v_l2 timestamptz := greatest(now() - interval '4 hours', v_midnight);
  v_l3 timestamptz := greatest(now() - interval '3 hours', v_midnight);
  v_l4 timestamptz := greatest(now() - interval '2 hours', v_midnight);
  v_l5 timestamptz := greatest(now() - interval '1 hours', v_midnight);
begin
  insert into public.lessons (id, center_id, service_id, teacher_id, student_id, starts_at, ends_at)
  values
