...
--       exercise_library_center_required: любая живая сессия (auth.uid() не
--       null), пишущая center_id => null, отбивается на уровне таблицы. Без
--       этого будущий grant insert «впрок» (как в 0036) или новая RPC, не
--       подставившая центр, создали бы упражнение, видимое всем центрам сразу
--       — не баг, а межцентровая утечка, и функция-фасад её не ловит.
...
as $$
begin
  -- auth.uid() is null — миграция/бэкфилл (сегодня платформенных строк нет,
  -- но место для них уже заведено 0036). Любая живая сессия пишет только в
  -- свой центр.
  if new.center_id is null and auth.uid() is not null then
