...
--       а не спека.
--
--   Р3. Для персонала null — только когда записей меньше 3 («мало
--       данных»), никогда не «есть данные, но паттерн не подошёл под
--       правило» — для этого случая есть отдельное явное значение
--       'stable'. Тот же класс ошибки, что student_balance.lessons_left
--       (null = и «безлимит», и «нет абонемента» — разбирались дважды
--       за сессию, не повторять). Родителю колонка не отдаётся вовсе
--       (Р4) — там null не про количество записей, а про роль; это
--       отдельная, не смешанная с Р3 причина null.
--
  ...
  return query
    select
      ...
      case
        ...
        -- gp.n is null недостижимо: left join lateral на агрегат без
        -- group by всегда даёт ровно одну строку (count(*) = 0 на
        -- пустом входе), просто defensive на случай будущей правки.
