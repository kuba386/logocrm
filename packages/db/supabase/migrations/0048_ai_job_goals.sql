...
--
--   Р2. В списке нет ничего, что идентифицирует ребёнка: ни имени, ни
--       плательщика, ни диагноза, ни custom_fields. Привязка к ребёнку
--       живёт только в базе; в OpenAI и в логи n8n уходит обезличенный
--       набор формулировок. target_date, last_score и note из
...
--       «добавим имя для контекста» падало тестом.
--
...
--       сообщения воркера (n8n/README.md).
--
...
--
--   Р6. limit 50 с отдельным ключом student_goals_total: молчаливое
--       усечение спрятало бы ровно ту цель, про которую диктовали. 50
--       активных целей у одного ребёнка — проблема данных, но воркер
--       обязан сказать специалисту, что список показан не весь.
--
--   Р7. Ответ ai_job_begin перестаёт быть набором идентификаторов и
--       становится клиникой. revoke переиздаётся явно, гранты закреплены
--       pgTAP-ассертами has_function_privilege (bot_worker — да; anon,
--       authenticated, public — нет): до 0048 грант этой функции не
--       проверял ни один тест. В n8n у воркфлоу poll должно быть выключено
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
         ...
         ) order by x.stage_sort, x.created_at, x.id), '[]'::jsonb)
    into v_goals
    from (
      select g.id, g.title, g.area, g.sound, st.title as stage_title, st.sort as stage_sort, g.created_at
       ...
       where g.student_id = v_request.student_id
         ...
         and g.deleted_at is null
         and g.status = 'active'
       order by st.sort, g.created_at, g.id
       limit 50
    ) x;
