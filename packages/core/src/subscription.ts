...
/**
 * Можно ли списать ещё одно занятие по остатку: безлимит — всегда,
 * allow_negative — всегда, иначе только при положительном остатке.
 *
 * НЕ зеркало attendance_fill_and_check — заморозка здесь не учитывается.
 * Результат нигде не используется для показа доступности в браузере (метки
 * идут из RPC, не отсюда), а заводить второй источник правды о заморозке
 * ради функции без потребителей — тот самый повод для расхождения, который
 * это же правило (CLAUDE.md, «Дублирование логики между SQL и TypeScript»)
 * и пытается предотвратить. Если появится потребитель, которому нужна
 * заморозка в браузере до ответа сервера, — тогда её сюда переносить.
 */
