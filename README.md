...
В Settings → Environments → **staging** нужен один секрет. Пока открыт
[supabase/supabase#44210](https://github.com/supabase/supabase/issues/44210)
— пулер отвергает пароль роли `postgres` после сброса, стабильно и во многих
регионах включая `ap-south-1`, — рабочий вариант только первый:

| Секрет | Где взять | Что даёт |
|---|---|---|
| `SUPABASE_ACCESS_TOKEN` | Supabase → аватар → Account → Access Tokens → Generate | CLI получает временную роль через API, пароль базы не нужен. Токен аккаунтный, поэтому ref проекта зашит в workflow |
| `SUPABASE_DB_PASSWORD` | Project Settings → Database → Reset | Строку собирает workflow. Доступ к одной базе, а не к аккаунту. Вернуться к нему, когда #44210 закроют |
| `SUPABASE_DB_URL` | Connect → Session pooler | Запасной путь, только через пулер |

Приоритет в этом же порядке: если задан токен, пароль и строка не читаются.
Держать стоит один секрет, а не все три.
