...
## Этап 7 — Клиника: диагностика, цели, ДЗ, AI-резюме ⬜ (7a/7c закрыты, 7b ждёт живой приёмки)

**Статус.** Разрезано на три части 19.09.2026 (согласовано владельцем, см. `reports/stage-7.md`). 7a — клиническое ядро без ИИ: таблицы, точечные RPC, `complete_lesson`, экран «Провести занятие», карточка ученика, библиотека упражнений — влито (0036–0040, PR #77–#86). 7b — голосовое резюме: код и бот влиты (0041/0042, PR #89–#91), сборка ветки `voice_summary` в n8n и живая приёмка с ключом OpenAI — за владельцем. 7c — Storage для видео ДЗ не понадобился: решением 21.09.2026 видео идёт в личный WhatsApp специалиста, а не через приложение; месячный отчёт (0043, PR #97), закрытие `attendance.comment` от родителя (0044, PR #98) и доставка уведомлений по ДЗ (0045, PR #99) влиты. Пункт чек-листа 3 ниже (голосовое резюме) ждёт живой приёмки 7b; пункты 4 и 6 закрыты.
...
4. ~~Родитель загружает видео ДЗ~~ — решением 21.09.2026 видео идёт в личный WhatsApp специалиста, Storage не используется (см. `reports/stage-7.md`). Проверить: родитель сдаёт ДЗ текстом (`submit_homework`) → специалист видит, пишет фидбек (`review_homework`) → родителю уведомление (0045).
