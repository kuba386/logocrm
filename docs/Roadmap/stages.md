...
| 8 | SaaS: тарифы, лимиты, онбординг, prod | ✅ (8b — живая приёмка прода за владельцем) |
...
**Статус.** Разрезано на три части 19.09.2026 (согласовано владельцем, см. `reports/stage-7.md`). 7a — клиническое ядро без ИИ (0036–0040, PR #77–#86). 7b — голос и ИИ на уровне базы (0041–0042), провайдер один — OpenAI. 7c — месячный отчёт (0043), уведомления по ДЗ (0045), видео к ДЗ решено вести через WhatsApp специалиста, Storage не нужен. Сверх плана: тренд по цели (0046), доставка резюме родителю и отказа диктовки специалисту (0047), панель заметок с «Утвердить» и экран отчёта за месяц (PR #103, #105). Живая приёмка 22.09.2026 пройдена владельцем целиком: голосовое → черновик с оценкой по цели (0048) → «Утвердить» → резюме родителю в Telegram (`sent` 08:05), отчёт за сентябрь родителю (`sent` 08:25). Подробности и находки — `reports/stage-7.md`, раздел «Живая приёмка 22.09.2026».
...
**Статус.** 8a (тарифы, лимиты, read-only, заявки на оплату, воронка, экспорт/удаление центра, публичная запись; 0049–0057) закрыта 24.09.2026 — `reports/stage-8.md`. 8b (prod-контур) закрыта 25.09.2026 — `reports/stage-8b.md`, чек-лист [ADR-010](../Decisions/ADR-010-production-checklist.md): Supabase `logocrm-prod` (`ap-south-1`, решение владельца), `deploy-prod.yml` по кнопке с одобрением, Vercel `logocrm-prod`, тот же бот и n8n на прод. Отложено до первого платящего центра: PITR, домен, Sentry, UptimeRobot. Живая приёмка прода (регистрация центра владельцем) — ещё нет. Мультифилиал (п. 5 промта) — этап 9+.
