...
Выручка и зарплата считаются от одной базы — маржа по специалисту
осмысленна. `planned` (отметили, но не нажали «Проведено») и `cancelled`
(отменили после отметок) не входят ни туда, ни туда.
...
- Ролевой фильтр в теле (`my_role() in ('owner','admin')` и свой центр): у
  ...
  агрегаты центра — не для них. Договорённость держит тест 0021,
  перечисляющий витрины `revenue_%`/`cash_%` из каталога — забытый фильтр
  в новой витрине роняет CI.
...
- Безлимит даёт `price_tiyin = 0` — в выручке это ноль, и он **виден**
  колонкой `unpriced_visits`. `visits` (на ребёнка) ≠ `lessons` (на занятие).
