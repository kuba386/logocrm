...
придёт из запроса/RPC, который будет читать историю в UI (задача ещё не
сделана для этого раздела); тот же пробел уже есть в смерженном UI 0066
(`syllable-assessment-panel.tsx`, `.order('date', {ascending:false})` без
тай-брейка на `created_at`) — чинить отдельной задачей.
