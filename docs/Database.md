...
**Цену считает SQL.** `assistant_finish(request, status, intent, model,
tokens_in, tokens_out, error)` берёт ставку из `ai_model_rates()`
(VALUES, тыйыны за 1M токенов — смена курса следующей миграцией),
неизвестная модель и токены сверх потолка — отказ; параметр «стоимость» от
сессии пользователя в реестр не принимается. `failed` — без расхода,
попытка не удаляется (удаление резерва = бесплатный обход квоты «сорви
вызов — лимит цел»); расхождение со счётом провайдера при обрыве после
ответа признаётся и измеряется числом failed-попыток. Один вопрос — ровно
один вызов провайдера (Vitest на `buildClassifyRequest`).
