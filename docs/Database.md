...
- **Индекс покрытия составного FK — БЕЗ `where`.** Партиальный индекс
  (`where deleted_at is null` и т.п.) не может обслужить проверку ссылочной
  целостности: RI-запрос Postgres при `delete` родителя не содержит этого
  предиката, планировщик его не возьмёт. Советник Supabase (`get_advisors`,
  `unindexed_foreign_keys`) на это НЕ ловит — партиальный индекс он
  засчитывает как покрытие, поэтому «советник молчит» не значит «FK
  покрыт» (0068 Р12 — построил партиальный индекс именно ради тишины
  советника, тишина оказалась ложной, снят 0069). Критерий, которому
  действительно верить: непартиальный btree-индекс, чьи ведущие колонки —
  все колонки FK в его порядке. Пример готовой проверки в pgTAP —
  `packages/db/supabase/tests/0069_fk_index_debt.test.sql`.
...
**Действия специалиста из бота (0071):** «Отметить» (посещение кнопками
со статусами центра) и «Заметка» (текст одним сообщением → черновик
`lesson_notes`, `soap.objective`). Контекст «чат ждёт следующего шага» —
`bot_pending_actions`: один живой на чат, кнопка несёт один uuid, второй
лежит в контексте (лимит 64 байта). Права — белым списком ролей и
перепроверяются на каждом шаге; занятие — только сегодняшнее в поясе
центра; read-only — явная `bot_assert_writable` (guard 0050 контур бота не
покрывает). Существующую отметку бот не переписывает (другой статус —
отказ «на экране»). `bot_today` с 0071 отдаёт `starts_local` (время в
поясе центра) и `can_mark`/`can_note`.

**Триггеры посещения эмитят через `emit_event_internal`** (0071 Р1): у
`emit_event` гейт `auth.uid()`, и из контура `bot_worker` любая отметка
падала бы 42501. `emit_event_internal` — без грантов вовсе, только из тел
definer-триггеров; для RPC остаётся `emit_event` (сессия) и
`emit_event_unchecked` (воркер без сессии). Заметка из бота остаётся
черновиком: утверждение требует `parent_summary` (0047 Р3), его пишет
человек на экране. Три забора вокруг текста заметки (0071 Р17/Р18):
«ждём диктовку» держится до записи черновика ИИ (`consumed_at` ставится
при приходе файла — до Whisper, поэтому одного `consumed_at is null`
мало), чужой черновик не дописывается (автора дописки в контуре бота не
фиксирует ничто), ответ на устаревший ForceReply-промпт отвергается по
`prompt_message_id` ↔ `reply_to_message` (иначе текст лёг бы в карту не
того ребёнка). `bot_pending_actions` — в deny-list экспорта: `chat_id` —
не данные центра.
...
### Анамнез — первый раздел речевой карты (0063)

`student_anamnesis` — профиль, не история: одна строка на ребёнка
(`unique(student_id)`, суррогатный `id` — иначе `audit_log` не адресуется
по `row_id`), собирается на первичке и дозаполняется. Беременность/роды,
ранние вехи развития и речи, заболевания, наследственность, условия
воспитания, слух/зрение на момент сбора. Родителю не видна ни в каком виде
(ADR-005) — чувствительнее, чем `sounds`/`speech_areas`: третьи лица,
наследственность.

Запись — единственный RPC `set_student_anamnesis(uuid, jsonb, timestamptz)`.
`jsonb`-патч с белым списком из 17 ключей вместо растущей от раздела к
разделу позиционной сигнатуры: ключ есть (даже со значением `null`/`""`) —
записать или снять поле, ключа нет — не трогать. Оптимистичная блокировка
по `updated_at` (`select ... for update`) — при правке существующей строки
`p_expected_updated_at` обязан совпасть, иначе `22023` без автоповтора.

Видимость — `clinical_student_visible(uuid)`, единая для всего
клинического блока (owner/admin, или teacher через `clinical_teacher_sees`);
`clinical_diagnostic_visible` (0059) с этой миграции — тонкая обёртка над
ней. Первое заполнение (анамнез чаще собирают ДО первого занятия) доступно
любому teacher центра; автор такой записи сохраняет к ней доступ и дальше,
даже без `clinical_teacher_sees` (приём 0038/0059 Р18) — но это исключение
не должно обходить проверку «ученик жив»: `created_by = auth.uid()` в
политике обязан идти через отдельную `student_alive(uuid)` (security
definer), а не через inline `exists(select ... from students)` — та
выполнялась бы от вызывающей роли и сама упиралась в RLS `students`
(`students_teacher_read_own` — только по `primary_teacher_id`), пряча
запись от собственного автора. Поймано только реальным CI, не ревью.

UI — `anamnesis-panel.tsx`: сводка заполненных полей всегда, форма —
по кнопке «Заполнить»/«Редактировать»; `key` формы — `updatedAt` строки,
иначе после сохранения React 19 не гарантирует, что неконтролируемые поля
покажут свежие значения с сервера.

### Артикуляционный аппарат — второй раздел речевой карты (0065)

`student_articulation` — той же формы профиль, что `student_anamnesis`
(0063), но ОТДЕЛЬНОЙ таблицей: иначе исключение автора анамнеза бесплатно
открыло бы и осмотр, `updated_at` двух разделов гасил бы правки друг друга
ложным «изменили параллельно», а `audit_log` перестал бы различать раздел.
Строение и подвижность губ/зубов/прикуса/нёба/языка/уздечки — фиксированные
клинические находки, не свободный текст.

Значения — латинские коды (`normal`/`limited`/`paretic`/…), не русские
слова в колонке (как `students.status`, `payments.kind`): правка
формулировки не должна быть миграцией данных. Русский текст — в
`apps/web/lib/errors.ts` (`CHECK_MESSAGES`), с ИМЕНОВАННЫМИ constraint —
безымянный автоконстрейнт при следующей правке набора значений сменит имя
молча, и `23514` перестанет матчиться. Список допустимых кодов живёт РОВНО
в одном месте — `CHECK` на колонке; `set_student_articulation` проверяет
только форму значения (`jsonb_typeof`), не членство в списке — две копии
списка расходятся, и RPC начинает отбивать код, который таблица бы
приняла, что хуже отсутствия проверки. Четыре поля, где находки реально
сочетаются (`lips_structure`, `teeth`, `soft_palate`, `tongue_structure`),
— `text[]` с `<@` (образец `notification_event_types.channels`, 0051), не
одно значение.

Круг доступа — `clinical_teacher_sees` ИЛИ назначенный `primary_teacher_id`
(`student_primary_teacher(uuid)`, security definer), СИММЕТРИЧНО чтению и
записи, не только «первому заполнению»: асимметричный вариант (primary_teacher_id
открывает только создание первой записи) воспроизводил тот же тупик, что
0063 Р10 чинила по CI — назначенный специалист без занятия видел бы пустую
страницу и тут же получал 42501, пытаясь её заполнить. Сравнение
`created_by = auth.uid()` в проверке прав — только через `coalesce(...,
false)`: при `created_by is null` (строки ещё нет, или заполнено мимо RPC)
голое `=` даёт SQL `NULL`, а `if not (... or NULL ...)` в plpgsql трактует
`NULL` как «не сработало» и пропускает `raise` — посторонний специалист
проходил бы проверку прав молча. Пойман только вторым проходом ревью, не
первым и не pgTAP до правки.

### Слоговая структура — третий раздел речевой карты (0066)

`syllable_assessments` — в отличие от анамнеза (0063) и артикуляционного
аппарата (0065), ИСТОРИЯ, не профиль (решение владельца 24.09.2026): новая
строка на каждое обследование, `deleted_at`/архивация, не правка на месте
— слоговую структуру пересдают, чтобы отследить прогресс. Смоделирована на
`diagnostics` (0059): та же связка `record_/update_/archive_`, а не
`set_*`-патч профилей. Таблица без префикса `student_` — история в проекте
всегда носит голое имя (`diagnostics`, `goals`, `homework`, `lessons`,
`attendance`); обратное правило («`student_*` всегда 1:1 профиль») не
универсально — `student_payers` тем же префиксом называет связку M:N.

`affected_classes` (коды `'1'..'14'`, 14 классов слоговой структуры по
А.К. Марковой) и `error_types` (`omission`/`permutation`/…) — ДВА
НЕЗАВИСИМЫХ `text[]`, не связка «какая ошибка на каком классе» через
junction-таблицу: решение владельца, проще и быстрее для первой версии,
соответствует тому, как это обычно фиксируют в бумажной речевой карте
(«нарушены классы 5, 7, 9; преобладающие ошибки: пропуск слога, упрощение
стечений»), без привязки ошибки к конкретному классу. `<@` держит
принадлежность набору, отдельный `cardinality(...) <= N` — верхнюю
границу (тот же образец `notification_event_types.channels`, 0051, но `<@`
не ловит дубли и не задаёт порядок сам по себе): `record_`/`update_`
дедуплицируют и сортируют массивы перед записью — числовой порядок для
классов (`order by length(x), x`, НЕ `::int`: каст на произвольном мусоре
падал бы 22P02 раньше, чем insert дойдёт до настоящего 23514 — поймано
только реальным CI, не ревью; лексикографический дал бы `'10'` раньше
`'2'`), алфавитный для типов ошибок. `record_` также требует хотя бы одно
содержательное поле (`affected_classes`/`error_types`/`conclusion`) —
иначе пустой вызов создавал бы запись, неотличимую на экране от
«обследован, нарушений нет». `conclusion` — тот же сентинел, что
`conclusion_code` в 0059: `''` снимает, `null` не трогает.

Restrictive-политика — точная копия образца 0063/0065:
`student_alive(uuid)` держит ВСЕХ, включая owner/admin, без исключений.
Первая версия плана исключала owner/admin, рассуждая, что архивация
ребёнка иначе стёрла бы историю с экрана владельца — рассуждение отклонено
на ревью написанного SQL: `students.deleted_at` нигде в приложении не
выставляется (`archive_student` пишет `status`, не `deleted_at`), сценарий
не наступает уже сегодня, а исключение снимало реальный слой защиты без
всякой компенсации — тот же класс дыры, что 0059 Р14 (permissive без
restrictive-backstop). Круг доступа на запись симметричен чтению и двум
другим разделам речевой карты на той же карточке (Р3): `clinical_teacher_sees`
ИЛИ `student_primary_teacher` ИЛИ авторство, не только «первое заполнение»
— иначе один экран вёл бы себя по-разному в трёх соседних разделах.
Ветка `teacher` в `update_syllable_assessment` держит `student_alive`
явно: `update_diagnostic` этого не делает (дыра, унаследованная не сюда),
автор проходил бы правку даже когда занятие отменили и ребёнок
архивирован.

Гранты — только `select` (запись строго через RPC), не `select+insert+update`,
как у `diagnostics` (0036): сознательное отличие, не недосмотр — у
`diagnostics` нет собственных инвариантов на insert мимо функции только
потому, что так сложилось исторически.

Дубль обследования за один день (двойной клик на «Сохранить» без
оптимистичного UI) сознательно не ограничен `UNIQUE(student_id, date)` —
клинически возможны два обследования в один день (утро/вечер, до и после
занятия), а форма и так не даёт повторно нажать раньше ответа сервера.
Родителю не видна ни в каком виде (ADR-005) — тот же класс данных, что
анамнез и артикуляция.

### Просодика — четвёртый раздел речевой карты (0067)

`prosody_assessments` — ИСТОРИЯ, как слоговая структура (0066), не профиль:
новая строка на каждое обследование. Форма колонок — как
`student_articulation` (0065), не как 0066: шесть отдельных категориальных
`text`-полей с `CHECK` на закрытый список кодов (`tempo`, `rhythm`,
`intonation`, `breathing`, `voice`, `logical_stress`), не массивы — находки
не комбинируются друг с другом внутри одного обследования (темп один, а не
несколько сразу). Код нормы — ОДИН И ТОТ ЖЕ `'normal'` во всех шести полях
(не `'expressive'` у интонации, не `'correct'` у ударения): вопрос «раздел
в норме?» из TS не должен собирать карту из шести разных литералов. `NULL`
— «не оценивалось», `'normal'` — «оценили, нарушений нет»: `NULL` как норма
отвергнут на плане ревью — строка, где дыхание не оценили, не должна
ЛОЖНО утверждать «дыхание в норме» (та же цена ошибки, что
`student_balance.lessons_left`, только здесь неоднозначность была бы у всех
шести полей сразу). `'monotone'` встречается и в `intonation`, и в `voice`
— подписи в UI обязаны быть per-field картой, не плоским `Record<code,
label>`.

Инвариант «хотя бы одно содержательное поле» — ИМЕНОВАННЫЙ CHECK на
таблице (`num_nonnulls(...) >= 1`, `prosody_assessments_not_empty`), не
только проверка в `record_`: в 0066 массивы (`not null default '{}'`)
сделали табличный CHECK неудобным, и guard остался только в RPC — но
полный-снимок форма правки даёт прямой путь стереть все поля через
`update_` в обход `record_` целиком. Здесь все поля скалярные — констрейнт
пишется в одну строку и держит дыру закрытой на уровне схемы. Сентинел
`''`/пробел → `null` — на ОБЕИХ функциях (`record_`/`update_`), для всех
семи текстовых полей: голый `select` «не выбрано» шлёт `''` и при создании
тоже, не только при правке; `nullif(trim(...), '')` ловит и пробел, не
только пустую строку — иначе случайный пробел в `conclusion` тихо обходил
бы `prosody_assessments_not_empty` (найдено вторым проходом ревью).

`student_alive(student_id)` в `update_prosody_assessment` — общее
предусловие для ВСЕХ ролей, включая owner/admin, не только ветки teacher
(найдено вторым проходом ревью: restrictive-политика уже держит эту
границу на чтение для всех без исключения, `update_` без такой же проверки
для owner/admin был бы ýже на чтение, чем на запись). `archive_` этой
проверки не получает сознательно — архивировать старую запись удалённого
ребёнка не запрещено.

Специалист в `p_teacher_id`/`my_teacher_id()` — не архивный
(`teachers.deleted_at is null`): проверка ОДНА, после ветвления
teacher/owner-admin, а не внутри одной из веток — `archive_teacher` (0017)
ставит только `teachers.deleted_at`, не трогая `membership` (отзыв доступа
— отдельное действие, `revoke_membership`, 0050), и путь `my_teacher_id()`
архивного специалиста иначе проходил бы молча, если владелец забыл отозвать
доступ отдельно (найдено вторым проходом ревью — первая редакция проверяла
только путь owner/admin).

Составной FK на `teachers` — `on delete set null (teacher_id)`, СО СПИСКОМ
колонок (0022), не голый `on delete set null` (без списка Postgres
зануляет ВСЮ составную пару, включая `center_id not null`, и любое
физическое удаление `teachers` падает загадочным not-null violation
вместо тихого каскада) — у 0036/0066 был тем же долгом, закрыт 0069.

Индекс `(student_id, date desc, created_at desc)` — два обследования в
один день (0066 разрешил это сознательно) не должны сортироваться
недетерминированно. Индекс САМ ПО СЕБЕ порядок не гарантирует — гарантия
приходит из запроса/RPC, который читает историю: `.order('date',
{ascending:false}).order('created_at', {ascending:false})` в `page.tsx`
(тот же пробел был в UI 0066 сразу после мержа — тай-брейк на `created_at`
добавлен туда же при добавлении UI 0067, PR с UI просодики). Для UI этого
раздела — та же дисциплина с самого начала, не постфактум.

Родителю не видна ни в каком виде (ADR-005) — тот же класс данных, что
анамнез и артикуляция.

### Чтение-письмо — пятый, последний раздел речевой карты (0068)

`reading_writing_assessments` — ИСТОРИЯ, как слоговая структура и
просодика, но сочетает ОБЕ формы данных, которые в 0066/0067 были
разделены: скалярные категориальные поля (как 0067) И два независимых
`text[]` (как 0066) — чтение и письмо оцениваются вместе на одном
обследовании, но имеют разную структуру находок. `reading_method`
(способ чтения: по буквам/по слогам/слого-словами/целыми словами) —
ОПИСАТЕЛЬНАЯ шкала прогрессии, у неё сознательно НЕТ кода `'normal'`, в
отличие от `reading_pace`/`reading_comprehension`/`writing_quality`:
способ чтения ожидаемо разный по возрасту, это не ось «норма/нарушение».
Вывод одной колонки из контракта 0067 «единый `normal` во всех
категориальных полях» держится не только на комментарии, но и на pgTAP-
заборе, который сверяет присутствие/отсутствие `'normal'` прямо в текстах
CHECK — иначе будущая функция «раздел в норме?», перебирающая текстовые
поля, пометила бы ребёнка, освоившего чтение целыми словами, как
нарушение.

`num_nonnulls()` объявлена `VARIADIC "any"` и МОЛЧА принимает массивы,
считая `'{}'` за non-null — сунуть `reading_errors`/`writing_errors`
внутрь `num_nonnulls` дало бы инвариант «хотя бы одно поле», который
ВСЕГДА истинен (не падает — врёт, что хуже отсутствия проверки). Формула
раздельная: `num_nonnulls(<пять скаляров>) >= 1 or
coalesce(cardinality(reading_errors),0) > 0 or
coalesce(cardinality(writing_errors),0) > 0` — `coalesce` держит инвариант
живым, даже если `not null` у массивов снимут следующей миграцией
(`false or NULL or false` = `NULL`, CHECK иначе пропускает такую строку
молча).

`writing_quality` (`normal`/`impaired`) — поле, которого не было в
исходном плане: без него `'{}'` в `writing_errors` означал бы ОДНОВРЕМЕННО
«не оценивалось» и «оценено, ошибок нет» (та же цена ошибки, что
`student_balance.lessons_left`) — у чтения такая ось есть
(`reading_comprehension`), у письма не было никакой. Отдельный именованный
CHECK (`reading_writing_assessments_writing_consistency`) запрещает
`writing_quality = 'normal'` одновременно с непустым `writing_errors` —
нашёл второй проход ревью: без него полный-снимок форма правки могла
сохранить «в норме» и список нарушений разом, если специалист забыл
переключить одно после правки другого.

Проверка архивного специалиста — ОДНА, после ветвления teacher/owner-
admin (урок 0067: первая редакция 0067 проверяла только путь owner/admin,
путь `my_teacher_id()` уволенного специалиста с живым `membership`
проходил молча). `student_alive` в `update_` — общее предусловие для ВСЕХ
ролей, включая owner/admin (урок 0067): restrictive-политика уже держит
эту границу на чтение для всех без исключения, запись не должна быть шире.
`archive_` этой проверки не получает — архивировать старую запись
удалённого ребёнка не запрещено.

`reading_errors` и `writing_errors` — РАЗНЫЕ множества кодов, хотя
некоторые текстовые коды совпадают (`'omission'`, `'permutation'`) и
пересекаются с `syllable_assessments.error_types` (`'substitution'`) —
каждый CHECK проверяет только свою колонку, это безопасно на уровне
схемы; риск — в будущем UI (общая TS-константа кодов, общий
`Record<code,label>`, общий префикс имени поля формы) — решать в задаче
на UI, не здесь. Верхние границы `cardinality` (6 и 8) равны числу кодов
в своих же `<@`-списках и после дедупа в RPC сегодня недостижимы — pgTAP
держит их самосогласованными явным забором, а не «на глаз».

Родителю не видна ни в каком виде (ADR-005).
