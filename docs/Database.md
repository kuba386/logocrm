...
| **Недоплата** по абонементу (всего) | `price_tiyin − paid_tiyin`, независимо от графика рассрочки — «сколько всего должны» | `subscription_payment_summary`, `installments_view`, `export_debts().subscriptions_unpaid_tiyin` (0018–0020, 0058) |
| **Просрочка** по абонементу | с планом рассрочки — сумма `installments_view.state = 'overdue'`; без плана — вся недоплата немедленно — «кому писать сегодня» | `student_balance.subscription_overdue_tiyin`, `student_subscriptions_overdue()` (0070) |

«Недоплата» и «просрочка» по абонементу — тоже не взаимозаменяемы, хоть и
похожи внешне. «Недоплата» отвечает на вопрос бухгалтера «сколько нам
вообще должны» и не смотрит на график: семья, оформившая рассрочку и
внёсшая первый платёж, уже недоплатила по этому определению, хоть следующий
платёж и наступит только через месяц. «Просрочка» отвечает на вопрос
администратора «кому писать сегодня» и специально исключает такую семью,
пока её собственный срок не наступил — 0070 завела отдельную функцию, а не
переиспользовала формулу `export_debts`, именно чтобы не звать родителей
раньше срока с живой страницы `/app/debts`.
